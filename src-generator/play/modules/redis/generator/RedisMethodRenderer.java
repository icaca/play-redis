...
/**
 * StringTemplate renderer for methods in the Redis connector class.
 * 
 * @author Tim Kral
 */
public class RedisMethodRenderer implements AttributeRenderer {
		...
		try {
			if ("name".equals(format)) {
				...
				if (paramNames.length < 1)
					throw new RuntimeException(String.format("Cannot get first parameter name for method: %s",
							redisMethod.getRawMethod().getName()));
	...
	private String renderParamList(CtMethod method, boolean withTypes) throws NotFoundException {
		...
		int numParams = 0;
		int totalParams = method.getParameterTypes().length;
		for (CtClass paramType : method.getParameterTypes()) {

			if (withTypes) {
				String type=renderType(paramType);
	...
	private String[] getMethodParamNames(CtMethod method) throws NotFoundException {
		...
		if (attr == null)
			throw new RuntimeException(String.format(
					"Class %s does not contain local variable table information. (Try using javac option -g)",
